...
exports.save_text = function (req, res) {
    ...
    query = con.query('UPDATE revistas SET nome_revista=?, titulo =?, texto =?, cargoS =?, nome_cria_txt =?, link =? WHERE id=? ', post, function (err, rows, fields) {
        console.log(query.sql);
        if (!err) {
            res.status(200).location(rows.insertId).send({
                "msg": "inserted with success"
...
exports.update_text = function (req, res) {
    ...
    query = con.query('UPDATE revistas SET nome_revista =?, titulo =?, nome_cria_txt=?, cargoS =?, texto =?,link =? WHERE id =?', update, function (err, rows,
        fields) {
        console.log(query.sql);
        if (!err) {
            console.log("Number of records updated: " + rows.affectedRows);
