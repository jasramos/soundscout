...
window.onload = function () {
    ...
    function show_revista() {
        ...
        async function fetchAsync() {
            ...
            let txt = "";
            const response = await fetch('/scoutSoundGet');
            const revistas = await response.json();
            txt += "<table class='table'>";
            txt += "<thead>";
            txt += "<tr><th>Nome</th><th>N de Edição</th><th>Revista</th><th>Link</th></tr></thead><tbody>";
     ...
     function save_revista() {
        ...
        data.link = document.getElementById("revista_revistas").value;
        console.log(data);
    ...
    function show_galeria() {
        ...
        async function fetchAsync() {
            ...
            let txt = "";
            const response = await fetch('/galeriaSound');
            const galeria = await response.json();
            txt += "<table class='table'>";
            txt += "<thead>";
            txt += "<tr><th>Titulo</th><th>Data</th><th>Fotgrafo</th><th>Revista</th></tr></thead><tbody>";
    ...
    function save_texto() {
        ...
        data.link = document.getElementById("link_texto").value;
    ...
    function creatlogin() {
        ...
        fetch('/signup', {
            ...
            if (!response.ok) {
                console.log(response.status); //=> number 100â599
                console.log(response.statusText); //=> String
                console.log(response.headers); //=> Headers
                console.log(response.url); //=> String
                throw Error(response.statusText);
                ...
                async function creatlogin() {
                    const res = await fetch('/signupSuccess');
            ...
            for (let index = 1; index <= txt; index++) {    
                ...
                fetch('/quiz/' + element)
                    ...
                    .then(response => {
                        ...
                        JSON.stringify(response);
                        // changed .value to .innerHTML but you can handle it as you wish
                        const txt = response[0].pergunta;
                        const txt1 = response[0].resposta1;
                        const txt2 = response[0].resposta2;
                        const txt3 = response[0].resposta3;
                        const txt4 = response[0].resposta4;
                        const txt5 = response[0].certa;

                        document.getElementById("q"+index).innerHTML = txt;
                        document.getElementById("a"+index).innerHTML = txt1;
                        document.getElementById("b"+index).innerHTML = txt2;
                        document.getElementById("c"+index).innerHTML = txt3;
                        document.getElementById("d"+index).innerHTML = txt4;
                        document.getElementById("r"+index).innerHTML = txt5;
